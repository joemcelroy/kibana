...
**The calling workflow must itself be managed.** An unmanaged parent can neither execute a managed child nor see globally-installed definitions, so a Worker registered outside `@kbn/workflows/managed` cannot reach the gate. A definition can waive that by declaring `callableByUnmanaged`, which the gate deliberately does not: it is what keeps a workflow the user can edit from proposing an action on its own behalf.
