...
import { EuiButton, EuiButtonIcon, EuiCopy, EuiFieldText, EuiFlexGroup, EuiFlexItem, EuiForm, EuiFormRow, EuiLink, EuiPanel, EuiText, EuiTitle, EuiToolTip } from '@elastic/eui'
...
const copyCloudIdHandler = (copy: () => void, sendTelemetry: ({ action, metric }: SendTelemetryHelper) => void ) => {
  return () => {
    copy()
    sendTelemetry({
      action: 'clicked',
        ...
        <EuiFlexItem>
          <EuiForm component="form">
            <EuiFormRow label={i18n.translate(
              ...
              <EuiFieldText onFocus={onFocusHandler} value={cloudId} compressed readOnly append={
                <EuiCopy textToCopy={cloudId}>
                  {(copy) => (
                    <EuiButtonIcon iconType={"copyClipboard"} onClick={copyCloudIdHandler(copy, sendEnterpriseSearchTelemetry)} iconSize="m" data-test-subj='CopyCloudIdButton' aria-label={i18n.translate(
